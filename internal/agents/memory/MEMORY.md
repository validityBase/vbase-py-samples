...
- Docs publishing installs only `docs/requirements/lock/base.txt` with pip hash checking enabled.
...
## Python Dependencies
- Runtime dependencies are declared in `requirements/src/base.in`; `setup.py` reads this input file instead of the generated hash-locked `requirements/lock/base.txt`.
- Generated lock files include hashes and are installed with `python -m pip install --require-hashes -r <file>`.
- `requirements/lock/tools.txt` pins pip-tools for lock regeneration.
